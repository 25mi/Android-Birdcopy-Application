...
public class MyLeesonsFragment extends Fragment implements MylessonsData.DealResult
{
    ...
    private void deleteLesson(BE_PUB_LESSON lessonData)
    {
        if (lessonData!=null){
            ...
            {
                MyApplication.getLeesonDownloadManager().closeAndReleaseDownloaderForID(lessonData.getBELESSONID());
            }

            //删除数据库本地纪录，资源自动释放
            new FlyingLessonDAO().deleteWithLessonID(lessonData.getBELESSONID());

            //删除相关本地内容
            if(lessonData.getLocalURLOfContent()!=null)
            {
                ShareDefine.deleteFile(lessonData.getLocalURLOfContent());
