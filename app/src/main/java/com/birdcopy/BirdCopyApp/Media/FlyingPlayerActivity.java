...
public class FlyingPlayerActivity extends Activity implements SurfaceHolder.Callback,
		...
		FlyingSubtitleView.OnTouchListener{
	...
	public void onCreate(Bundle savedInstanceState)
	{
		...
		//字幕相关
		String subFilePath =FlyingFileManager.getLessonSubFilePath(mContentId);
    ...
    */
	...
	private void parseSubtitile()
	{
		...
		if(FlyingFileManager.fileExists(subFilePath)){
			...
			try{
				InputStream is = new FileInputStream(FlyingFileManager.getFile(subFilePath));
				subTitle = new FlyingSrtParser().parse(is);
			}
			catch (Exception e)
			{
				//
	...
	private void updateSubtitleTimerFired(long time)
	{
		...
		Cue cue = subTitle.getCue(freshTimeInSeconds);

		if(cue!=null && cue.text!=null)
		{
			subtitleView.setText(cue.text);
		}
		else
		{
